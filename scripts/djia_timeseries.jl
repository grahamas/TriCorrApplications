...
let (λ_n, λ_t) = (8,10), recalculate=true,
    ...
    task_name = "djia_seqcont_A_snip01_start_$(Dates.format(start_date,"yyyy_mm_dd"))_lags_$(λ_n)_$(λ_t)",
    task_time = Dates.format(Dates.now(), "yyyy_mm_dd-HHMMSS");

plots_subdir = plotsdir("$(task_name)_$(task_time)_snippet_01")
mkpath(plots_subdir)
...
contributions, contributions_dates = if recalculate == true
    ...
    contributions, contributions_dates = calc_contributions_timeseries_snippet_01(djia, trading_dates, λ_n, λ_t; n_bootstraps = 0)
